...
<figure>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 280" class="w-full max-w-2xl mx-auto my-6 rounded-lg shadow-md" role="img" aria-label="ML extrapolation failure: neural networks flatten out beyond training range while linear models continue the trend">
    <rect width="600" height="280" fill="#f8f8fc"/>
    <g font-family="Inter, sans-serif" font-size="11" fill="#374151">
      <text x="40" y="22" font-weight="600" fill="#0a0a0a">Why ML Models Fail at Extrapolation</text>
      ...
      <text x="120" y="55" font-size="10" fill="#64748b">Training range</text>
      <text x="350" y="55" font-size="10" fill="#dc2626" font-weight="600">Outside training range</text>
      ...
      <text x="430" y="95" font-size="10" font-weight="600" fill="#1e40af">NN: flattens</text>
      <text x="430" y="60" font-size="10" font-weight="600" fill="#d4af37">Linear: continues trend</text>
      <g fill="#1e40af">
      ...
      <text x="60" y="265" font-size="9" fill="#64748b" font-style="italic">Neural networks produce averaged, flat predictions outside their training range</text>
    </g>
  </svg>
  <figcaption>Why ML models fail at extrapolation. Inside the training range (left of the dashed line), a neural network (blue) follows the data closely. But beyond it (right of the dashed line), the network's output flattens to a near-constant value — it has no learned pattern to extend. A linear model (gold dashed), by contrast, continues its trend indefinitely. This fundamental limitation affects all interpolator-style models: they can blend known examples but cannot invent new ones.</figcaption>
</figure>
