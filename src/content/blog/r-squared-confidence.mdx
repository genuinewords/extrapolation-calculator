...
This approach aligns well with understanding [linear extrapolation](/blog/linear-extrapolation.mdx/) as a baseline: start with the simplest reasonable model and only add complexity when the data and domain knowledge justify it.
...
As discussed, a higher R² can result from overfitting rather than genuine explanatory power. A linear model with R² = 0.88 that reflects a real physical relationship is far more valuable for extrapolation than a degree-5 polynomial with R² = 1.00 that merely memorizes the training data. This overfitting problem is especially pronounced in machine learning — see [extrapolation in machine learning](/blog/extrapolation-machine-learning.mdx/) for why ML generalization beyond training data is so challenging.
